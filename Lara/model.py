...
print(np.shape(test_attribute_vec))
test_G_user = sess.run(fake_user_emb, feed_dict={attribute_id:test_attribute_vec})
print(np.shape(test_G_user))

p_at_10,p_at_20,M_at_10,M_at_20,G_at_10,G_at_20 = support.test(test_item_batch, test_G_user)
print('推荐的前10个用户：precision: {} | mean average precision: {} | ndcg: {}'.format(p_at_10, M_at_10, G_at_10))
print('推荐的前20个用户：precision: {} | mean average precision: {} | ndcg: {}'.format(p_at_20, M_at_20, G_at_20))
...
for it in range(800):
    ...
    for D_it in range(D_range):
        ...
        print('判别器loss: {}'.format(D_loss_now))
    ...
    for G_it in range(G_range):
        ...
        print('生成器loss: {}'.format(G_loss_now))
...
#        print( test_G_user[:10])
        ...
        if it%50 == 0:
            print('the best: ')
            print('推荐的前10个用户：precision: {} | mean average precision: {} | ndcg: {}'.format(max_p_at_10, max_M_at_10, max_G_at_10))
            print('推荐的前20个用户：precision: {} | mean average precision: {} | ndcg: {}'.format(max_p_at_20, max_M_at_20, max_G_at_20))
    # if it % 100 == 0:
